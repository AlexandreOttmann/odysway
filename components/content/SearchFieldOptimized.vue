...
// Use the server-side search data composable
const {
  destinations,
  regions,
  searchFieldContent,
...
// Memoized destinations to regions mapping
...
function searchFn() {
  ...
  gtag('event', 'search', {
    ...
    travelType: `${travelTypeChoice.value}`,
    from: `${date.value[0]}`,
    to: `${date.value[1]}`,
  })
...
// Watch for route changes and handle parameters
